...
// =====================================================
// 2. MAIN AUTH PAGE (SLIDING & LOGIC)
// =====================================================
export default function AuthPage() {
  const router = useRouter()
  const [isSignUp, setIsSignUp] = useState(false) // State untuk kontrol sliding
  const [loading, setLoading] = useState(false)
  const [error, setError] = useState('')

  // Form States (Gabungan Login & Register)
  const [formData, setFormData] = useState({
    username: '', // Hanya dipakai saat Register
    email: '',
    password: '',
    confirmPassword: '' // Hanya dipakai saat Register
  })
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data.user) {
        // Cek apakah profil sudah ada/lengkap
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const { data, error: signUpError } = await supabase.auth.signUp({
        ...
        password: formData.password,
        options: {
          // PENTING: Simpan username ke metadata agar bisa dipanggil di Dashboard
          data: {
      ...
      if (data.user) {
        ...
        setIsSignUp(false) // Geser otomatis ke panel Login
        setFormData({ ...formData, password: '', confirmPassword: '' }) // Reset password
      }
    } catch (err: any) {
  ...
  return (
    <div className="relative min-h-screen flex items-center justify-center p-4 overflow-hidden font-sans text-gray-800">
      ...
      <div className="relative bg-white/80 backdrop-blur-2xl border border-white/50 rounded-[2rem] shadow-2xl w-full max-w-[1000px] min-h-[600px] overflow-hidden flex">
        ...
        {/* ================================================= */}
        <div className={`absolute top-0 left-0 h-full w-full md:w-1/2 flex flex-col items-center justify-center p-8 transition-all duration-700 ease-in-out z-10 ${isSignUp ? 'translate-x-full opacity-0 pointer-events-none' : 'opacity-100'}`}>
          <div className="w-full max-w-sm">
            {/* Header Login */}
            <div className="text-center mb-8">
              <div className="flex items-center justify-center gap-2 mb-2">
                <img src="/logo.png" alt="Logo" className="w-10 h-10 rounded-xl object-cover shadow-md" />
                <span className="text-2xl font-bold bg-clip-text text-transparent bg-linear-to-r from-emerald-600 to-teal-600">
                  NutriCalc+
                </span>
              </div>
        ...
        {/* ================================================= */}
        {/* PANEL KANAN: FORM REGISTER (Hidden Default) */}
        {/* ================================================= */}
        <div className={`absolute top-0 left-0 h-full w-full md:w-1/2 flex flex-col items-center justify-center p-8 transition-all duration-700 ease-in-out z-10 ${isSignUp ? 'translate-x-0 opacity-100' : '-translate-x-full opacity-0 pointer-events-none'}`}>
          <div className="w-full max-w-sm">
            {/* Header Register */}
            ...
            <form onSubmit={handleRegister} className="space-y-4">
              ...
              <div className="relative">
                ...
                <input 
                  type="text" 
                  placeholder="Username (Panggilan)" 
        ...
        {/* ================================================= */}
        <div 
          className={`hidden md:block absolute top-0 left-1/2 w-1/2 h-full overflow-hidden transition-transform duration-700 ease-in-out z-50 rounded-l-[50px] ${isSignUp ? '-translate-x-full rounded-l-none rounded-r-[50px]' : ''}`}
        >
          {/* Background Gradient Hijau */}
          <div className={`bg-linear-to-r from-emerald-500 to-teal-600 text-white relative -left-full h-full w-[200%] transform transition-transform duration-700 ease-in-out flex items-center justify-center ${isSignUp ? 'translate-x-1/2' : 'translate-x-0'}`}>
            
            {/* Panel Kiri (Terlihat saat mode Login -> Menawarkan Daftar) */}
            <div className={`w-1/2 h-full flex flex-col items-center justify-center px-12 text-center transform transition-transform duration-700 ease-in-out ${isSignUp ? 'translate-x-0' : '-translate-x-[20%]'}`}>
            ...
            {/* Panel Kanan (Terlihat saat mode Register -> Menawarkan Masuk) */}
            <div className={`w-1/2 h-full flex flex-col items-center justify-center px-12 text-center transform transition-transform duration-700 ease-in-out ${isSignUp ? 'translate-x-[20%]' : 'translate-x-0'}`}>
